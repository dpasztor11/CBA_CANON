...
const int maxlen=6;
const int len=6;
const int cskCount = len == 6 ? 843 : (len == 5 ? 7 : 4);
...
std::vector<std::pair<std::string, bool>> csks;
std::map<std::string, std::string> canonsMap;



int main() {
    // getting CSKEq from file
    std::ifstream cskEqFile;
    cskEqFile.open("txt/reducedComplementCBARaw" + std::to_string(len) + ".txt");
    std::string temp;

    for (long long i = 0; i < cskCount; i++)
    {
        cskEqFile >> temp;
        csks.push_back({temp, false});
    }
    ...
    canonFile.open("txt/canons" + std::to_string(len) + ".txt");
...
//        std::cout<<"cba.size"<<cba.size().first()<<","<<(int)cba.size().second()<<"\n";
        ...
        if (len<=maxlen-2) {
            ...
            if (map[len+2].try_emplace(canon,std::move(res)).second) {
                ...
                std::cout << str << " maps to " << canonsMap[str] << std::endl;
            }
        }
...
//                std::cout<<"Processing 0,1 on "<<i<<"\n";
                ...
                if (map[len+1].try_emplace(canon,std::move(res)).second) {
                    ...
                    auto expold = canonize(cba, len);
                    auto expnew = expand_canon(canon);
 /*                   for(unsigned int k=0; k<expnew.size(); k++) std::cout<<(int)expnew[k]<<",";
                    std::cout<<"  Created from: ";
                    for(unsigned int k=0; k<expold.size(); k++) std::cout<<(int)expold[k]<<",";
   ...
   /*                     for(unsigned int k=0; k<expnew.size(); k++) std::cout<<(int)expnew[k]<<",";
    ...
    for(auto const& el: map[5]) {
        auto canon=el.first;
        auto cba=el.second;
        for(unsigned int i=0; i<canon.size()-1; i++) std::cout<<(int)canon[i]<<","; 
