    ...
    for(auto vec : colouring_bit_array_internal::comparators[len].relevant_indices_absolute) 
        if (cba[ColouringBitArray::Index(vec)]) {
            for(uint_fast8_t j=0;j<len;j++) {
                for(uint_fast8_t c=0;c<3;c++) if (c!=vec[j]) {
                    bool violation=true;
                    for(uint_fast8_t k=0;k<len;k++) if (j!=k) 
                        if (colouring_bit_array_internal::kempe_switch_exists(cba, vec, {j, k}, c, vec[j])) {
                            violation =false; break;
                        }
...
bool is_kempe_closed(const ColouringBitArray &cba, uint_fast8_t len) {
    for(auto vec : colouring_bit_array_internal::comparators[len].relevant_indices_absolute) 
        if (cba[ColouringBitArray::Index(vec)]) 
            for(uint_fast8_t c1=0;c1<3;c1++) for(uint_fast8_t c2=c1+1;c2<3;c2++) {
                std::vector<int> indices;
                for(unsigned int i=0;i<vec.size();i++) if (vec[i]==c1 || vec[i]==c2) indices.push_back(i);
                ...
                for(const auto &matching : matchings) {
                    ...
                    for(auto small_matching: ps) {
                        ...
                        if (!cba[ColouringBitArray::Index(vec2)]) {all_ok=false;break;}
                    }
                    if (all_ok) {violation=false; break;}
                }
                if (violation) return false;
            }
