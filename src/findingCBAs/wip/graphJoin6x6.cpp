...
int main()
{
    ...
    for (long long i = 1; i < foundCsk6AsStrings.size(); i++)
    {
        ...
        for (int danglingEdges = 4; danglingEdges <= 6; danglingEdges++)
        {
            int cbaCount = (danglingEdges == 6 ? (i + 1) : (danglingEdges == 5 ? cskEq5.size() : cskEq4.size()));
            int connectWaysBegin = (danglingEdges == 6 ? 0 : (danglingEdges == 5 ? 4 : 7));
            int connectWaysEnd = (danglingEdges == 6 ? 3 : (danglingEdges == 5 ? 6 : 9));
    ...
    {
        foundCSKFileOutput << allCsk6AsStrings[i] << " " << foundCskMap[allCsk6AsStrings[i]] << "\n";
    }
